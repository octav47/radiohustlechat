    ...
    socket.onmessage = function (e) {
        ...
        if (data.type == 'server') {
            switch (data.info) {
                ...
                case 'success':
                    document.getElementById('send').childNodes[0].nodeValue = 'Отправить';
                ...
                case 'update':
                    showChat('info', null, data.user.oldun + ' сменил имя на ' + data.user.un);
                    clients[data.user.id] = data.user;
                    break;

                case 'connection':
                    var userip = data.user.ip ? ' [' + data.user.ip + ']' : '';
                    showChat('info', null, data.user.un + userip + ' присоединился к нам.');

                    clients[data.user.id] = data.user;
                    document.getElementById('users').innerHTML = Object.keys(clients).length + ' пользователей';
                    break;
                ...
                case 'spam':
                    showChat('global', null, 'Подождите хотя бы одну секунду перед отправкой нового сообщения. Вы получаете предупреждение. Получено предуждений: ' + data.warn + ' из 5');
                    break;
        ...
        if (username === undefined) {
            ...
            switch (command[0].toLowerCase()) {
                ...
                case 'clear':
                    ...
                    showChat('light', 'System', 'Сообщения удалены');
                    break;
                ...
                default:
                    showChat('light', 'Error', 'Неизвестная команда, воспользуйтесь помощью /help');
                    break;
            }
        } else {
